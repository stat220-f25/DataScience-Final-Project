...
sleep_cor   <- cor(sleep_numeric,   use = "pairwise.complete.obs")
student_cor <- cor(student_numeric, use = "pairwise.complete.obs")

# 7. Simple linear model

# For now, here is a valid model using ONLY student variables:
grade_model <- lm(
  performance_index ~ hours_studied + sleep_hours,
  data = student |> drop_na(performance_index, hours_studied, sleep_hours)
)
model_summary <- tidy(grade_model)
```
